...
def parse_opt(known=False):
    ...
    # --------------------------------------------------- 常用参数 ---------------------------------------------
    parser.add_argument('--weights', type=str, default=ROOT / 'weight/best.pt', help='initial weights path') # weights: 权重文件
    parser.add_argument('--cfg', type=str, default='models/yolov3_mask.yaml', help='model.yaml path')  # cfg: 网络模型配置文件 包括nc、depth_multiple、width_multiple、anchors、backbone、head等
    ...
    parser.add_argument('--epochs', type=int, default=5)  # epochs: 训练轮次
    parser.add_argument('--batch-size', type=int, default=24, help='total batch size for all GPUs, -1 for autobatch') # batch-size: 训练批次大小
